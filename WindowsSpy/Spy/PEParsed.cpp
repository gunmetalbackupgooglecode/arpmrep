...
PEParsed::PEParsed(DWORD _fileMapAddress) :
  fileMapAddress(_fileMapAddress)
{
  ...
  headerFreeSpaceAddress = 0;
  headerFreeSpaceSize = 0;
  isXCodeInHeader = false;

  imageImportDescriptor = NULL;
  numberOfImportedDlls = 0;
  maxFreeSpaceIATAddress = 0;
}
...
char* PEParsed::GetLastError()
{
  return lastError;
}


DWORD PEParsed::RvaToFileOffset(DWORD rva)
{
...
DWORD PEParsed::FileOffsetToRva(DWORD fileOffset)
{
  for (int sectionIndex = 0; sectionIndex < imageFileHeader->NumberOfSections; ++ sectionIndex)
  {
    IMAGE_SECTION_HEADER curSection = imageSectionHeader[sectionIndex];
